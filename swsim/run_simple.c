...
#define KEEP_LINES 32
#define RTL_MAX_LINE 200
char last[KEEP_LINES][RTL_MAX_LINE];
unsigned last_p = 0;

int get_rtl_commit(unsigned *pc, unsigned *wbr, unsigned *wbv)
{
        static unsigned prev_stall = 0;
        char buf[RTL_MAX_LINE];
        unsigned time = ~0, hazzard = ~0, flush = ~0, stall = ~0,
                d_pc = ~0, d_inst = ~0,
                x_pc = ~0, x_arg0 = ~0, x_arg1 = ~0, x_dst = ~0,
                m_pc = ~0, m_res = ~0, m_dst = ~0,
                w_pc = ~0, w_res = ~0, w_dst = ~0;
        ...
        for (;;) {
                ...
                r = sscanf(buf,
                           "%uz %x %x/%x D %x:%x X %x:%x:%x>%x M %x:%x>%x W %x:%x>%x\n",
                           &time, &hazzard, &flush, &stall,
                           &d_pc, &d_inst,
                           &x_pc, &x_arg0, &x_arg1, &x_dst,
                           &m_pc, &m_res, &m_dst,
                           &w_pc, &w_res, &w_dst);
...
void note_commit(unsigned pc, unsigned wbr, unsigned wbv)
{
        ...
        if (enable_cosimulation) {
                unsigned rtl_pc, rtl_wbr, rtl_wbv;
                int r = get_rtl_commit(&rtl_pc, &rtl_wbr, &rtl_wbv);
                if (!r) {
                ...
                if (rtl_pc != pc || rtl_wbr != wbr || rtl_wbv != wbv) {
                        ...
                        for (i = 0; i < KEEP_LINES; ++i) {
                                unsigned k = (i + last_p) & (KEEP_LINES - 1);
                                if (last[k])
                                        printf("%s", last[k]);
                        }
...
void run_simple(MIPS_state_t *state)
{
        ...
        for (;;) {
                ...
                case REG: // all R-type, thus rd is target register
                        ...
                        case MULT: {
                                u_int64_t u64 = (int64_t)s * (int64_t)t;
                                state->lo = u64;
                                state->hi = u64 >> 32;
                                break;
                        }
                        ...
                        case DIVU:
                                ...
                                break;
                        case ADD:  assert(0); wbv = s + t; break; // XXX Ignore the trap on overflow for now
                        case ADDU: wbv = s + t; break;
                        case SUB:  assert(0); wbv = s - t; break;
